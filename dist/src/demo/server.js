...
async function buildHarnessResponse(body) {
    const interventionMode = body.interventionMode ?? "calm";
...
function isValidHarnessRequest(body) {
    return Boolean(body.profileId &&
        body.prompt &&
...
async function handler(request, response) {
    ...
    if (request.method === "POST" && url.pathname === "/api/judge") {
        ...
        const result = await runPoqo(body.prompt, body.profileId);
