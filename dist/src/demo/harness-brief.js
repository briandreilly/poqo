...
function pickModeVariant(interventionMode, variants) {
...
function buildNarrowingLines(result, interventionMode) {
...
function buildDomainLockLines(domainAnchor) {
    if (!domainAnchor) {
        return [];
    }
...
export function buildPromptGuideLines(runtimeGuide) {
...
export function buildPoqoBrief(result, runtimeGuide, interventionMode, domainAnchorOverride = null) {
    ...
    if (result.move === "DIRECT") {
        if (result.analysis.signals.violenceRisk) {
            ...
            lines.push(pickModeVariant(interventionMode, {
