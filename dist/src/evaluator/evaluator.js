...
const varianceProfiles = ["default", "founder", "kidsafe"];
export async function loadPromptSet(label = "v1") {
    const raw = await readFile(path.join(process.cwd(), benchmarkManifest[label].filePath), "utf8");
    return JSON.parse(raw);
}
function assessResponseQuality(promptCase, result) {
    const response = result.finalResponse;
    const normalized = response.toLowerCase();
    const hasBranching = /\bif\b/.test(normalized) ||
        /smallest next question:/i.test(response) ||
        /^\d+\./m.test(response);
    const hasConcreteProgress = /part of this is answerable now|the likely answer depends on|keep the hard constraint|working answer|basis:|smallest useful answer|use plain html|single local config file/i.test(response);
    const genericClarify = /can you provide more details|provide more details|tell me more|need more information|more information is needed/i.test(normalized);
    ...
    const overNarrowed = (result.move === "NARROW" && promptCase.expectedMove === "DIRECT") ||
        (result.move === "NARROW" && !stalled && !hasBranching && !hasConcreteProgress);
    const overProved = (result.move === "PROVE" && promptCase.expectedProof === "none") ||
...
function buildSummary(results, promptTotal = results.length) {
...
function buildEvaluationResult(promptCase, profileId, result, profileDrift = false, driftReason) {
    return {
        promptCase,
        profileId,
...
function buildReport(label, results, promptTotal) {
...
function applyVarianceDriftChecks(results) {
...
async function evaluateVariancePromptSet() {
...
export async function evaluatePromptSet(label = "v1") {
    if (label === "variance") {
...
export async function evaluateAllPromptSets() {
    const reports = await Promise.all(benchmarkOrder.map((label) => evaluatePromptSet(label)));
    return {
        label: "all",
        reports,
        summary: buildSummary(reports.flatMap((report) => report.results), reports.reduce((sum, report) => sum + report.summary.promptTotal, 0))
    };
}
