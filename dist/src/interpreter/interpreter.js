...
const CONSTRAINT_PATTERN = /\b(must|must not|should|should not|need to|needs to|under|within|no |without|only|avoid|keep it|budget|local-only|local only|offline|simple|fewest dependencies|single developer|dead simple|tiny|smallest|this week|by friday)\b/i;
...
const WORLD_PATTERN = /\b(latest|current|today|price|prices|lts|law|legal|rules|regulation|requirements|compliant|coppa|app store|license|maintenance|cite|citation|source|news|statistics|right now)\b/i;
const CHAT_PATTERN = /\b(requirements above|constraints above|already stated|this chat|our conversation|as discussed|current scope|locked in|we talked through|we already said|we agreed on|we already agreed|we already locked in|we already nailed down|we've already decided|we've already locked in|we've already nailed down|we already decided|earlier)\b/i;
const MATERIAL_REFERENCE_PATTERN = /\b(attached|below|above|pasted|paste|transcript|contract|policy|document|spec|resume|clause|excerpt|line below)\b/i;
const EXPLICIT_PROOF_PATTERN = /\b(show .*basis|show what .* stands on|prove|proof|cite|citation|source|based on what|what .* based on)\b/i;
...
const ANSWERABLE_SUBJECTIVE_PATTERN = /^(are|is)\b.+\b(scary|creepy|too hard|hard)\b[?.!]*$/i;
const DIRECT_QUESTION_START_PATTERN = /^(what|where|when|why|how|who|which)\b/i;
const SUBJECTIVE_SAFETY_PATTERN = /\b(gun|knife|weapon|kill|killing|hurt me|hurt them|blood|gore|self-harm|suicide|abuse|unsafe|dangerous|adult)\b/i;
const VIOLENCE_RISK_PATTERN = /\b(violent|violence|revolution|hurt(?:ing)? them|attack(?:ing)? them|force)\b/i;
const VIOLENCE_SUPPORT_PATTERN = /\b(answer|only way|justified|solve it|seems like|would solve it|what then)\b/i;
...
const STRONG_UNSUPPORTED_STANCE_PATTERN = /\b(garbage|trash|useless|insane|propaganda|ruined|destroyed|filthy|will never work|never work|never works|set back\b.*\b(years?|decades?)\b|failure|disaster)\b/i;
const BROAD_TRAIT_REALITY_CLAIM_PATTERN = /(?:\b(?:is|are)\s+(?:an?\s+)?(?:honest(?:\s+person)?|dishonest|liar|fraud|corrupt|immoral|safe for everyone|safe for all|safe|unsafe|reliable|unreliable|trustworthy|untrustworthy)\b|\b(?:lacks|lack)\s+moral\s+character\b|\bhas\s+no\s+moral\s+character\b|\bscams everyone\b)/i;
const CONTESTED_FRAMING_DOMAIN_PATTERN = /\b(nazi|nazis|fascism|fascist|communism|communist|socialism|socialist|left(?:-wing)?|right(?:-wing)?|authoritarian|movement|regime|party|political labels?|labels?)\b/i;
const CONTESTED_FRAMING_CUE_PATTERN = /(?:people\s+(?:nowadays\s+)?(?:get|define)\s+[^.?!]{0,60}\s+wrong|it(?:['’]s| is)\s+nonsense\s+to\s+think|\breally\b|\bactually\b|because\s+it\s+was\s+in\s+the\s+name|\bin\s+the\s+name\b|\bbasically\s+the\s+same\b|\bmean\s+the\s+opposite\b|\bmeans\s+the\s+opposite\b|\bwrong\s+now\b)/i;
const COHERENT_CRITIQUE_LEAD_PATTERN = /\b(is a failure|is broken|is a mess|is failing(?:\s+people|\s+kids)?|is failing people|is failing kids|is a disaster|is a scam|are a scam|is a waste of money|are a waste of money|is declining|are declining|is hurting|are hurting)\b/i;
const BROAD_CRITIQUE_SUBJECT_PATTERN = /\b(healthcare system|justice system|school system|public schools|schools|college|climate(?: change)? policies|healthcare|product|app|city|housing|transit)\b/i;
...
const ARGUMENT_LOADED_STATEMENT_PATTERN = /\b(is just|are just|there(?:['’]s| is) nothing|nothing intelligent|anyone saying otherwise|doesn['’]?t understand|don't understand|hyping it|waste of money)\b/i;
const HEDGED_STANCE_PATTERN = /\b(i think|i don['’]?t think|i do not trust|i don['’]?t trust|to me|for me|in my experience)\b/i;
const STANCE_BASIS_PATTERN = /\b(because|since|after|when|for example|for instance|due to|based on|what happened|the reason|from what)\b/i;
const PERSONAL_SCOPE_PATTERN = /\b(i['’]?ve (?:ever )?(?:been to|had|seen|played|used)|in my experience|for me|to me|from what i saw|i['’]?ve ever|i['’]?ve had|i['’]?ve seen|i['’]?ve played|i['’]?ve used)\b/i;
const VAGUE_CONCERN_PATTERN = /^(this feels off|something is wrong here|something feels wrong|this is weird|that seems wrong|this does not seem right|this doesn't seem right|i do not like this|i don't like this|i am not sure about this|i'm not sure about this)\b[.!?]*$/i;
const NON_ACTIONABLE_REMARK_PATTERN = /^(?:ew|wow,? that is gross|that was rude|(?:he|she|they|that guy|this guy|that girl|this girl|that person|this person)\s+(?:is|looks|has)\s+(?:ugly|gross|disgusting|huge boobs?))\b[.!?]*$/i;
const BOUNDED_DECLARATIVE_STATEMENT_PATTERN = /^(?!(?:this feels|this seems|that seems|this is weird|something\b|i am\b|i['’]m\b))(?:[a-z][a-z'’:-]*)(?:\s+[a-z][a-z'’:-]*){0,3}\s+(?:is|are|was|were)\s+(?:an?\s+|the\s+)?[a-z][a-z'’:-]*(?:\s+[a-z][a-z'’:-]*){0,3}[.!?]*$/i;
const QUESTION_AS_CONSTRAINT_PATTERN = /^(should i|should we|should this|can i|can we|could i|could we|would i|would we|do i|do we|is this)\b/i;
const INCOMPLETE_FRAGMENT_PATTERN = /^[a-z0-9']+(\s+[a-z0-9']+)?[!.?]*$/i;
function normalizeWhitespace(input) {
    return input.replace(/\s+/g, " ").trim();
}
function extractQuotedMaterials(input) {
...
function extractConstraints(input) {
    const segments = input
        ...
        .map((part) => part.trim())
        .filter(Boolean);
    const constraints = segments.filter((segment) => CONSTRAINT_PATTERN.test(segment) && !QUESTION_AS_CONSTRAINT_PATTERN.test(segment));
    return Array.from(new Set(constraints)).slice(0, 6);
}
function classifyUtteranceType(normalized, materials) {
    const violenceRisk = VIOLENCE_RISK_PATTERN.test(normalized) && VIOLENCE_SUPPORT_PATTERN.test(normalized);
    const proofRequest = EXPLICIT_PROOF_PATTERN.test(normalized) ||
        CHAT_PATTERN.test(normalized) ||
        MATERIAL_REFERENCE_PATTERN.test(normalized) ||
        materials.length > 0;
    const decisionRequest = VAGUE_PRIORITY_PATTERNS.some((pattern) => pattern.test(normalized)) ||
        /\b(should i|should we|recommend|pick|choose|prioriti[sz]e|build first|ship first|now or later)\b/i.test(normalized);
    const factualRequest = WORLD_PATTERN.test(normalized) ||
...
function buildSignals(normalized, materials, utteranceType) {
    const shortPrompt = normalized.split(/\s+/).length <= 4;
    const broad = BROAD_PATTERNS.some((pattern) => pattern.test(normalized));
    const multiPart = (normalized.match(/\band\b/gi)?.length ?? 0) >= 2 ||
        (normalized.match(/,/g)?.length ?? 0) >= 2;
    const multiAssertion = multiPart || normalized.split(/[.!?]+/).filter((part) => part.trim().length > 0).length >= 2;
    const citesChatContext = CHAT_PATTERN.test(normalized);
    const stackDecisionWithoutCriteria = /\b(pick a stack|which stack|best stack|stack fits best)\b/i.test(normalized) &&
        !citesChatContext &&
        !/\b(database|auth|typescript|node|react|budget|team|constraint|requirements|above|offline|simple)\b/i.test(normalized);
    const choiceWithoutCriteria = /\b(choose|pick|recommend|recommendation|should i|should we|monthly or annual|react or plain html|react or svelte|html or react|email or qr|json or sqlite|sqlite or files|which .* fits best)\b/i.test(normalized) &&
    ...
    const compareWithoutCriteria = /\b(compare|versus|vs)\b/i.test(normalized) &&
        !citesChatContext &&
    ...
    const priorityDecisionWithoutContext = VAGUE_PRIORITY_PATTERNS.some((pattern) => pattern.test(normalized)) && !citesChatContext;
    const answerableSubjectiveQuestion = ANSWERABLE_SUBJECTIVE_PATTERN.test(normalized) && !SUBJECTIVE_SAFETY_PATTERN.test(normalized);
    const directQuestionReady = DIRECT_QUESTION_START_PATTERN.test(normalized) &&
    ...
    const vagueConcernReaction = VAGUE_CONCERN_PATTERN.test(normalized) &&
        !personallyScopedEvaluation &&
        !violenceRisk;
    const nonActionableRemark = !normalized.endsWith("?") &&
        NON_ACTIONABLE_REMARK_PATTERN.test(normalized) &&
        !vagueConcernReaction &&
        !violenceRisk;
    const contestedFramingClaim = !normalized.endsWith("?") &&
        CONTESTED_FRAMING_DOMAIN_PATTERN.test(normalized) &&
    ...
    const broadCritiqueLead = COHERENT_CRITIQUE_LEAD_PATTERN.test(normalized) &&
        BROAD_CRITIQUE_SUBJECT_PATTERN.test(normalized);
    const coherentCritiqueBundle = !normalized.endsWith("?") &&
        ((multiAssertion &&
            COHERENT_CRITIQUE_LEAD_PATTERN.test(normalized) &&
        ...
        !violenceRisk;
    const argumentLoadedStatement = !normalized.endsWith("?") &&
        multiAssertion &&
        ...
        !violenceRisk;
    const strongUnsupportedStance = !normalized.endsWith("?") &&
        (STRONG_UNSUPPORTED_STANCE_PATTERN.test(normalized) ||
        ...
        !violenceRisk &&
        !EMOTIONAL_REACTION_PATTERN.test(normalized);
    const boundedDeclarativeStatement = !normalized.endsWith("?") &&
    ...
    const refersToMissingSourceText = /\b(rewrite|summarize|edit|review|analyze)\s+this\b/i.test(normalized) ||
    ...
    const materialReferencedButMissing = (MATERIAL_REFERENCE_PATTERN.test(normalized) || refersToMissingSourceText) &&
        !documentProvided &&
        !citesChatContext;
    const worldFactSensitive = !utteranceDirectReady &&
        !vagueConcernReaction &&
        !nonActionableRemark &&
        !contestedFramingClaim &&
        !coherentCritiqueBundle &&
    ...
    const scopeConflict = /\bunder\s+\d+\s+words\b.*\bevery detail\b/i.test(normalized) ||
    ...
    const missingKeyVariable = (shortPrompt && !answerableSubjectiveQuestion && !directQuestionReady && !violenceRisk && !vagueConcernReaction && !nonActionableRemark && !contestedFramingClaim && !coherentCritiqueBundle && !argumentLoadedStatement && !strongUnsupportedStance && !utteranceDirectReady) ||
        vagueConcernReaction ||
        contestedFramingClaim ||
        coherentCritiqueBundle ||
        argumentLoadedStatement ||
...
function buildAmbiguity(signals) {
    const reasons = [];
    if (signals.broad) {
...
function buildProofNeed(normalized, signals, constraints, materials) {
    const reasons = [];
...
function buildReadiness(signals, ambiguityReasons) {
    const reasons = [];
    if (signals.materialReferencedButMissing) {
...
export function interpretInput(rawInput) {
