import type { Move, PoqoResult, ProofSelection, RequestAnalysis, RuntimeGuide } from "../types.js";

function isFramePreservingDirectAnalysis(analysis: RequestAnalysis): boolean {
  return (
    !analysis.signals.directQuestionReady &&
...
export function isFramePreservingDirect(analysis: RequestAnalysis, move: Move): boolean {
  return move === "DIRECT" && isFramePreservingDirectAnalysis(analysis);
}

function pickTopic(input: string): string {
  return input
...
function chooseNames(topic: string): string[] {
  const words = topic
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, "")
    .split(/\s+/)
  ...
  const pool = Array.from(
    new Set(["Poqo", "Scope", "Basis", "Next", "Signal", "Local", "Plain", ...words.map((word) => word[0].toUpperCase() + word.slice(1))])
  );

  return [0, 1, 2].map((index) => `${pool[index]}${pool[index + 1] ?? ""}`);
}

function directCore(analysis: RequestAnalysis): string {
...
function inferMissingVariable(analysis: RequestAnalysis): string {
  const input = analysis.normalizedInput;
...
function branchHint(analysis: RequestAnalysis): string[] {
  const input = analysis.normalizedInput;
...
function smallestNextQuestion(analysis: RequestAnalysis): string {
  if (analysis.signals.priorityDecisionWithoutContext) {
...
function narrowCore(analysis: RequestAnalysis): string {
  ...
  if (
    analysis.signals.compareWithoutCriteria ||
...
function inferFromDocument(analysis: RequestAnalysis): string {
...
function proveCore(analysis: RequestAnalysis, proof: ProofSelection): string {
...
function applyTermSubstitutions(body: string, runtimeGuide: RuntimeGuide): string {
...
function labelForMove(runtimeGuide: RuntimeGuide, move: Move): string {
...
function applyProfilePresentation(runtimeGuide: RuntimeGuide, move: Move, body: string): string {
...
export function buildResponse(
  ...
  const constraintLine =
    analysis.constraints.length > 0 && move !== "PROVE"
...
export function buildPoqoResult(
  runtimeGuide: RuntimeGuide,
  analysis: RequestAnalysis,
  move: Move,
  proof: ProofSelection,
