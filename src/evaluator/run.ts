...
import type { BenchmarkId, EvaluationReport, EvaluationResult, PromptSetMode } from "../types.js";

function formatPercent(value: number): string {
  return String((value * 100).toFixed(1)) + "%";
}

function failureReasons(result: EvaluationResult): string[] {
...
async function main(): Promise<void> {
  const mode = (process.argv[2] ?? "v1") as PromptSetMode;
